...
const poll = {
  ...
  registerNewAnswer() {
    ...
    // 1.2
    promptValue < 4 && promptValue >= 0
      ? this.answers[promptValue]++
      : alert('Invalid number, please choose a number between 0 and 3');
  ...
  registerNewAnswer1() {
    ...
    typeof answer === 'number' &&
      ...
      this.answers[answer]++;
    this.displayResults1();
    this.displayResults1('string');
  },

  displayResults1(type = 'array') {
    if (type === 'array') {
      console.log(this.answers);
    } else if (type === 'string') {
...
poll.displayResults1.call({ answers: [5, 2, 3] }, 'string');
poll.displayResults1.call({ answers: [1, 5, 3, 9, 6, 1] });
