...
const dogsJulia = data.julia1;
const dogsKate = data.kate1;

const checkDogs = function (dogsJulia, dogsKate) {
  // 1.
  const newDataJulia = dogsJulia.slice(1, -2);
  //   2.
  const newArray = newDataJulia.concat(dogsKate);
  console.log(newArray);
  //   for (const [i, age] of newArray.entries()) {
  newArray.forEach(function (dog, i) {
    //   3.
    console.log(
      `"Dog number ${i + 1} is ${
...
// 4.
checkDogs(data.julia2, data.kate2);
...
const calcAverageHumanAge = function (ages) {
  ...
  const excludeUnder18s = humanAges.filter(age => age > 18);

  // const averageAge =
  //   excludeUnder18s.reduce((acc, age, i) => acc + age, 0) /
  //   excludeUnder18s.length;

  const averageAge = excludeUnder18s.reduce(
    (acc, age, i, arr) => acc + age / arr.length,
    0
  );

  console.log(humanAges);
  console.log(excludeUnder18s);
  console.log(averageAge);
  return averageAge;
};
// Codingchallenge 3
const calcAverageHumanAge1 = ages => {
  const averageAge1 = ages
    .map(age => (age <= 2 ? 2 * age : 16 + age * 4))
    .filter(age => age > 18)
    .reduce((acc, age, i, arr) => acc + age / arr.length, 0);
  return console.log(averageAge1);
};
...
// 1.
dogs.map((_, i) => (dogs[i].recommendedFood = dogs[i].weight ** 0.75 * 28));
console.log(dogs);
//////////////////////////////
// Jonas
...
dogs.map((_, i) => {
  if (dogs[i].owners.includes('Sarah'))
    return dogs[i].curFood > dogs[i].recommendedFood
...
// 4.
const str = ownersEatTooMuch.flat().map((cur, i, arr) => {
  console.log(`${cur} and`);
  return cur;
});
console.log(str);
...
// 5
dogs.map((_, i) =>
  dogs[i].recommendedFood === dogs[i].curFood
    ? console.log(true)
    : console.log(false)
);
//////////////////////////////
